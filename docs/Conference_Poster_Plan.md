...
> **主張較正（[CLAIM_CALIBRATION.md](CLAIM_CALIBRATION.md) 準拠）**: 本発表は肉離れの**発生確率**や**予防効果**を実証するものではない。評価対象は**injury-related mechanical-load surrogate**（正規化筋線維長・能動/受動/腱張力・伸張性仕事）であり、候補動作は**性能制約下で負荷代理指標を下げる実験的介入候補**として提示する。形態×骨盤の主張は交絡があるため探索的とする。

## 発表の主軸

本発表は、骨盤前傾そのものの再確認ではなく、**予測シミュレーションを使って、ハムストリングの力学的負荷代理指標（fascicle-strain injury surrogate）と最高速度の速度-負荷トレードオフを設計し、選手タイプごとの候補介入を比較する**ことを主軸にする。

推奨タイトル案:

> 予測スプリント最適化による性能維持下のハムストリング力学的負荷代理指標の速度-負荷パレート設計

短い副題案:

> 筋束長・筋力の違いに応じた「技術 vs トレーニング」候補の探索的比較
...
### H2: 短筋束選手では技術修正が高コスト（探索的仮説）

**注意（[CLAIM_CALIBRATION.md](CLAIM_CALIBRATION.md) C3）**: 形態（筋束長）と骨盤姿勢は交絡しており、以下は**主要結論ではなく探索的仮説**として提示する。

最適筋束長が短い選手では、筋束ひずみが構造的に高く、走り方だけで負荷代理指標を下げるには大きな速度損失を要する**可能性がある**。一方、筋束長を標準へ戻すトレーニング経路は、速度を落とさずに負荷代理指標を下げる**傾向が見られた**（探索的）。
