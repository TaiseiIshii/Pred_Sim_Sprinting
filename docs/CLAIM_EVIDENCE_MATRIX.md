...
Provenance: **analysis_commit `bb0433a`**; engine `ham_load_metrics.py` v1.0.0. The historical
`manifest.csv` `commit` column = `repo_head_at_runtime 3da75fc`; per-condition commits in
`manifest_provenance.csv` (see [PROVENANCE.md](PROVENANCE.md)).
...
| A6 | A biarticular-overstretch penalty yields **candidate motions that reduce the load surrogate at near-matched performance** (w=0.1: surrogate −4.1%, speed −0.24% within the pre-declared 0.5% numerical performance-matching tolerance); the knee point is w=0.2 (−7.0% surrogate, −0.61% speed). **N=50; N=100 multi-start confirmation pending (plan Phase 2).** | [pareto_nominal.csv](../Results/Validation_Master/pareto_nominal.csv); [fig_p1](../Results/Validation_Master/fig_p1_pareto.png) | **PASS** (N=50) |
