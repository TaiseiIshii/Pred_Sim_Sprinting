# シミュレーションは疫学の“既知”を機序で再現できるか
### ― ハムストリング肉離れ（HSI）疫学のリスク因子と予測シミュレーションの整合（構成概念妥当性）―
...
- **主要な結果（8件中6件が一致・1件は精緻化・1件は対象外）**:
  ...
  2. **筋束長リスク閾値 ≈10.56 cm**（Timmins 2016）と量的に接続：モデルの BFlh 最適筋束長 **L0=11.8 cm**、スイープは **8.3–14.2 cm** で臨床閾値を**またぐ**。
  ...
  5. **BFlh が最も傷つく筋**（Woods 2004; Askling 2007）を再現：二関節ハムのうち **BFlh の能動的伸張性仕事が最大（16.5 J）**。
...
- **意義**: 疫学は「短い筋束＝高リスク」を**相関**でしか示せない。シミュレーションは同じ関連を**因果的な機序**（同一走課題で筋束が短いほど相対ひずみが増える）として再現し、しかも**閾値レンジ（≈10.6 cm）まで量的に一致**する。これは、本モデルが単なる曲線当てはめではなく、**疫学の背後にある力学を捉えている**ことの強い証拠である。

> **結論**: 予測シミュレーションは、独立に確立された HSI 疫学の**主要な可変リスク因子と介入方向を機序として再現**する。したがって、本モデルは疫学的知見と整合した“構成概念妥当性”を備え、**個別化された予防仮説（誰に何を）**を計算機内で検証する土台となる。
