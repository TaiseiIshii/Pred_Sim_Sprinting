...
% ---- Figures ----------------------------------------------------------
% NOTE: Japanese-labelled figures for the report/guide are generated by
% analysis/plot_pelvic_shift_figs.py (matplotlib + Yu Gothic). MATLAB titles
% below are kept in ASCII/English to avoid font mojibake on JP Windows.
...
title('Manipulation check: realised mean vs imposed offset'); grid on;
...
title('Hamstring peak normalised fibre length: dose-response'); legend(hamNames,'Location','best'); grid on;
...
xlabel('offset (deg)'); ylabel('peak Fpetilde'); title('Passive force'); legend(hamNames,'Location','best'); grid on;
subplot(1,2,2);
for h=1:4
...
xlabel('offset (deg)'); ylabel('eccentric impulse \Sigma Fce\cdotmax(0,vM)\cdotdt'); title('Eccentric load'); grid on;
...
xlabel('offset (deg)'); title('Mechanism: touchdown hip flexion -> ham MTU length'); grid on;
subplot(1,2,2);
...
xlabel('offset (deg)'); title('Task cost: speed and muscle effort'); grid on;
