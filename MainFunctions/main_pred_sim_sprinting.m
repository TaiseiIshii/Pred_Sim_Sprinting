...
pathUtilityFuncs = [pathmain, '\UtilityFunctions\'];
contPrmsFile = fullfile(pathUtilityFuncs, 'Sph_Plane_simultOptContPrms_Fmax_2_Vmax_12.mat');
if ~exist(contPrmsFile,'file')
    % Fallback to path based on this script location
    scriptDir = fileparts(mfilename('fullpath'));
    projectRoot = fileparts(scriptDir);
    contPrmsFile = fullfile(projectRoot, 'UtilityFunctions', 'Sph_Plane_simultOptContPrms_Fmax_2_Vmax_12.mat');
end
if ~exist(contPrmsFile,'file')
    error('Contact parameters file not found: %s', contPrmsFile);
end
% Workaround for possible non-ASCII path issues
tempContDir = fullfile(tempdir,'pred_sim_sprinting_tmp');
if ~exist(tempContDir,'dir')
    mkdir(tempContDir);
end
tempContFile = fullfile(tempContDir, 'Sph_Plane_simultOptContPrms_Fmax_2_Vmax_12.mat');
if ~exist(tempContFile,'file')
    copyfile(contPrmsFile, tempContFile);
end
contPrms = load(tempContFile);
mu_s = 0.95;
mu_d = 0.3;
mu_v = 0.3;
tv   = 0.001;
...
fprintf('Loading external DLL functions...\n');

try
    F_cont_v21 = external('F_cont_v21','Spr_Imp_GRFs_ownCont_V21.dll');
    fprintf('[OK] F_cont_v21 loaded\n');
catch ME
    fprintf('[WARNING] Failed to load F_cont_v21: %s\n', ME.message);
    fprintf('This may affect simulation functionality.\n');
    F_cont_v21 = [];
end

try
    F_cont_ana21 = external('F_cont_ana21','Spr_Imp_GRFs_ownCont_V21an.dll');
    fprintf('[OK] F_cont_ana21 loaded\n');
catch ME
    fprintf('[WARNING] Failed to load F_cont_ana21: %s\n', ME.message);
    F_cont_ana21 = [];
end
...
modelFile = [modelpath osimModelName];
try
    muscProperties = extractMuscProperties(modelFile,muscleNames(1:end-3));
catch
    % Workaround for Java failing to open paths with non-ASCII characters
    tempModelDir = fullfile(tempdir,'opensim_model_ascii');
    if ~exist(tempModelDir,'dir')
        mkdir(tempModelDir);
    end
    tempModelFile = fullfile(tempModelDir,osimModelName);
    if ~exist(tempModelFile,'file')
        copyfile(modelFile,tempModelFile);
    end
    muscProperties = extractMuscProperties(tempModelFile,muscleNames(1:end-3));
end
...
optimumOutput1 = saveOptimumFiles(scaling1,Options,optVars_sc1,optVars_nsc1,pred_timeGrid_con,pred_timeGrid,...
    ...
    function [kinProcess,timeGrid,timeNodes,h,statesNames] = processIKoutput(statesFname,kinSF,Options,tau_root,d,nq,modelpath,osimModelName,pathExpData)
        ...
        for i = 1:nq.all
            ...
            pp_1 = local_ppder(pp,1);
            qdot_aux(:,i) = ppval(pp_1,timeGrid);
            pp_2 = local_ppder(pp,2);
    ...
    function ppd = local_ppder(pp, n)
        % Compute nth derivative of a pp-form spline without Curve Fitting Toolbox
        if nargin < 2
            n = 1;
        end
        ppd = pp;
        for k = 1:n
            if ppd.order <= 1
                ppd.coefs = zeros(ppd.pieces,1);
                ppd.order = 1;
                break;
            end
            m = ppd.order;
            factor = (m-1:-1:1);
            ppd.coefs = ppd.coefs(:,1:end-1) .* factor;
            ppd.order = m - 1;
        end
    end
